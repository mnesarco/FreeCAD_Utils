...
def t_proxy_set_prop_mode(overridden: Any, meta: TypeMeta):
    def handler(self, *, mode: PropertyEditorMode, name: str = None, binding: str = None):
...
def t_proxy_set_prop_status(overridden: Any, meta: TypeMeta):
    def handler(self, *, status: str, name: str = None, binding: str = None):
        if not (name or binding):
            raise NameError("name or binding argument must be provided")
        if name and binding:
            raise NameError("name and binding arguments are mutually exclusive")
...
def t_view_proxy_get_def_dm(overridden: Any, meta: TypeMeta):
    ...
    if len(defaults) == 0:
        if len(meta.display_modes) == 1:
            ...
            raise RuntimeError(f"One display mode must be declared as default: {modes}")

    if len(defaults) > 1:
        raise RuntimeError(f"Only one display mode can be declared as default: {defaults}")
...
for _property_type in _FC_PROPERTY_TYPES:
    globals()[f"Property{_property_type}"] = _prop_constructor(f"App::Property{_property_type}")
del _property_type
...
# ──────────────────────────────────────────────────────────────────────────────
def _basic_modal_dlg(message: str, title: str = "Message", details: str = None) -> QMessageBox:
    """
